            ...
            <div class="houses">
                <div class="room" v-for="(house,item) in equipmentHouses" :key="item" v-if="tabCard">
                    ...
                    <div class="cells" :houseId="house.houseId" v-if="!entire">
                        ...
                        <room v-for="(room, index) in house.rooms" :key="index" :room="room" :houseId="house.houseId" :houseFormat="houseFormat" class="cell" @view="showDrawer" @sendFloor="sendFloor" @click.native="show(house.houseId,room.id)" />
                    </div>
					<!-- 整幢显示 -->
					<div class="cells" :houseId="house.houseId" v-if="entire">
                        <room v-for="(room, index) in house" :key="index" :room="room.rooms[0]" :house="room" :houseId="room.houseId" :houseFormat="houseFormat" class="cell" @view="showDrawer" @sendFloor="sendFloor" @click.native="show(house.houseId,room.id)" />
                    </div>
                </div>
				<!-- 整租显示 -->
				<div v-if="!tabCard" class="shareHouse ">
					<div style="padding:10px 0 0 10px" class="flexc wrapHouse">
						<span v-for="(house,lista) in equipmentHouses" :key="lista">
							<room v-for="(room, index) in house.rooms" :key="index" :room="room" :houseId="house.houseId" :houseFormat="houseFormat" :house="house" class="cell" @view="showDrawer" @sendFloor="sendFloor" @click.native="show(house.houseId,room.id)" />
						</span>
					</div>
				</div>
            </div>

<template lang="html">
    <div class="subjectHouseLocation">
        <el-table :data="subjectFlows" :row-key="getRowKeys" :expand-row-keys="expands" @row-click='handleRowHandle' ref="tableData" class="cursorp">
                ...
                <template slot-scope="props">
                    <div class="housesInLocation">
                        <div v-for="item in props.row.housesInLocation" class="flexc">
                            ...
                            <div>
                                <span>{{price(item.rent)}}</span>
                            </div>
                            <div>
                                <span>{{price(item.rentFee)}}</span>
                            </div>
                            <div>
                                <span>{{price(item.topup)}}</span>
                            </div>
                            <div>
                                <span>{{price(item.topupFee)}}</span>
                            </div>
                            <div>
                                <span>{{price(item.finalReceive)}}</span>
                            </div>
                            <div>
                                <span>{{price(item.finalPay)}}</span>
                            </div>
                            <div>
                                <span>{{price(item.balance)}}</span>
                            </div>
                        </div>
                    </div>
                </template>
                        ...
                        .then(res => {
                            ...
                            res.forEach((ele, num) => {
                                this.$model('housedetail')
                                    ...
                                    .then(data => {
                                        this.$set(this.subjectFlows[index].housesInLocation[num],
                                            'location', data)
                                        if (num === 0) {
                                            if (row.id !== this.upflowi) {
                                                this.toggle(row.id)

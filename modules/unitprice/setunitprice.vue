<template>
  <div>
    ...
    <div class="copyHouseExtra" v-if="copyHouseExtra">
      <copyExtra :data="equipmentHouses" @refresh="copyCost" :houseFormat="houseFormat"/>
    </div>
      ...
      equipmentHouses: function () {
        if (this.houseFormat === 'SHARE') {
          return _.flatten(this.homePrice.map((element, index) => {
            ...
            return fp.reject(h => h.houseId === fp.get('houseId')(this.priceSource))(_.flatten(_.concat(element, _.pick(element, ['rooms']).rooms).map(ele => {
              return _.assignIn(ele, new Foo)
            })))
          }))
        }
        if (this.houseFormat !== 'SHARE') {
          return fp.reject(h => h.houseId === fp.get('houseId')(this.priceSource))(this.homePrice.map((element, index) => {
            if (element.prices.length !== 0) {
              element.electricity = []
              element.prices.map((item, list) => {
                if (item.type === 'ELECTRIC') {
